...
Цей репозиторій містить мої домашні роботи з курсу **FullStack** від школи **iTHillel**.

## Що знаходиться в репозиторії

Кожне домашнє завдання розміщене в окремій папці. Головна навігація винесена на сторінку `index.html`, з якої можна перейти до кожної роботи.

## Структура

У репозиторії зараз доступні такі домашні завдання:
...
Тема роботи: **SVG, A11Y, DevTools, Project optimization**.

Що реалізовано:

- створено SVG-полотно з квадратом, прямокутником, колом та еліпсом;
- для кожної фігури задано власні кольори заливки та контуру товщиною `5px`;
- для еліпса додано CSS-анімацію зі зміною кольору заливки та обводки;
- додано мінімальний адаптивний дизайн і базову доступність через `title`, `desc` та семантичну структуру;
- на сторінці розміщено посилання на GitHub Pages і на папку з кодом у репозиторії.
...
## Домашнє завдання №22 (JavaScript Essentials)

Тема роботи: **Замикання, Math-методи, рекурсія, винятки, стрілкові функції, use strict, this та accessors**.

Що реалізовано:

- створено окрему сторінку `home-work-22C.D_js/index.html` для презентації виконаного домашнього завдання;
- стилі винесено в `home-work-22C.D_js/styles.css`, а логіку та приклади виконання розміщено в `home-work-22C.D_js/Main.js`;
- на основі шаблону реалізовано `counter`, `counterFactory`, `myPow`, `myMax`, `myMul`, `myDouble` і `myTriple`;
- додано приклади використання замикань, рекурсії, `Math.max.apply()`, `bind()` та обробки помилок через `try...catch...finally`;
- у HTML і CSS додано українські коментарі над блоками та пояснення для нових тегів.
...
- результати оновлюються в реальному часі з debounce-затримкою, видимою крутилкою завантаження та скасуванням попереднього запиту через `AbortController`;
- кожна картка фільму показує назву, рік випуску, тип, постер і посилання на IMDb;
- додано обробку порожнього запиту, замалої кількості символів, `Too many results.`, відсутніх результатів, відсутнього постера та помилок API;
- картки рендеряться через HTML-шаблон `template`, який JavaScript наповнює даними з API;
- у коді застосовано destructuring, spread, rest і optional chaining;
- у HTML і CSS додано українські коментарі над блоками та пояснення для нових тегів.
...
1. Відкрийте головний файл `index.html`.
2. Оберіть потрібне домашнє завдання в списку.
3. Перейдіть за кнопкою на сторінку відповідної роботи.
